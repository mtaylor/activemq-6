 ...
 */
package org.apache.activemq.common.example.hornetq;
 ...
 * topic.
 */
public class HornetQInitialContextFactory implements InitialContextFactory
{
   ...
   protected HornetQConnectionFactory createConnectionFactory(String name, Hashtable environment) throws URISyntaxException, MalformedURLException
   {
    ...
    */
   protected HornetQConnectionFactory createConnectionFactory(Hashtable environment) throws URISyntaxException, MalformedURLException
   {
      ...
      if (environment.containsKey(Context.PROVIDER_URL))
      {
         ...
         if (providerURI.getScheme().equals(TCP_SCHEME))
         {
            ...
            {
               connectionFactory = HornetQJMSClient.createConnectionFactoryWithoutHA(getJmsFactoryType(environment), transportConfigurations);
            }
         }
         else if (providerURI.getScheme().equals(UDP_SCHEME))
         {
            DiscoveryGroupConfiguration dgc = new DiscoveryGroupConfiguration()
               .setRefreshTimeout(transportConfig.containsKey(REFRESH_TIMEOUT) ? Long.parseLong((String) transportConfig.get(REFRESH_TIMEOUT)) : HornetQClient.DEFAULT_DISCOVERY_REFRESH_TIMEOUT)
               .setDiscoveryInitialWaitTimeout(transportConfig.containsKey(DISCOVERY_INITIAL_WAIT_TIMEOUT) ? Long.parseLong((String) transportConfig.get(DISCOVERY_INITIAL_WAIT_TIMEOUT)) : HornetQClient.DEFAULT_DISCOVERY_INITIAL_WAIT_TIMEOUT)
            ...
            {
               connectionFactory = HornetQJMSClient.createConnectionFactoryWithHA(dgc, getJmsFactoryType(environment));
            }
            else
            {
               connectionFactory = HornetQJMSClient.createConnectionFactoryWithoutHA(dgc, getJmsFactoryType(environment));
            }
         }
         ...
         else if (providerURI.getScheme().equals(VM_SCHEME))
         {
            ...
            connectionFactory = HornetQJMSClient.createConnectionFactoryWithoutHA(getJmsFactoryType(environment), tc);
         }
         else
         {
   ...
   private void invokeSetter(Object target, final String propertyName, final Object propertyValue)
   {
      ...
      try
      {
         if (setter != null)
         {
            HornetQClientLogger.LOGGER.info("Invoking: " + setter + " that takes a " + setter.getParameterTypes()[0] + " with a " + propertyValue.getClass());
      ...
      {
         HornetQClientLogger.LOGGER.warn("Caught exception during invocation of: " + setter, e);
      }
   }
}
